...
class NotifMirrorService : NotificationListenerService(), BluetoothCommunicationManager.BluetoothMessageListener {
    ...
    private fun buildForegroundNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, "mirror_service")
            .setContentTitle("Notif Mirror Active")
    ...
    override fun onListenerConnected() {
        ...
        try {
            mediaSessionManager?.addOnActiveSessionsChangedListener(
                sessionListener,
                ComponentName(this, NotifMirrorService::class.java)
            )
            updateMediaControllers(mediaSessionManager?.getActiveSessions(ComponentName(this, NotifMirrorService::class.java)))
        } catch (e: Exception) {
    ...
    override fun onMediaActionReceived(action: String) {
        ...
        val controller = controllers.find { it.playbackState?.state == PlaybackState.STATE_PLAYING } 
                        ...
                        ?: mediaSessionManager?.getActiveSessions(ComponentName(this, NotifMirrorService::class.java))?.firstOrNull()
    ...
    private fun shouldMirror(sbn: StatusBarNotification, blacklisted: Set<String>): Boolean {
        ...
        // Only mirror notifications that are clearable, not ongoing, and not group summaries
    ...
    private suspend fun StatusBarNotification.toNotificationData(): NotificationData? = withContext(Dispatchers.Default) {
        try {
            ...
            val icon = try {
                val userContext = try { createPackageContext(packageName, 0) } catch (e: Exception) { this@NotifMirrorService }
    ...
    private suspend fun drawableToBitmap(drawable: Drawable): Bitmap = withContext(Dispatchers.Default) {
        if (drawable is BitmapDrawable) {
            ...
            // Ensure the BitmapDrawable bitmap also has transparency preserved
        ...
        bitmap.eraseColor(android.graphics.Color.TRANSPARENT) // clear to transparent, not black
    ...
    companion object {
        private const val TAG = "PhoneService"
    }
}
